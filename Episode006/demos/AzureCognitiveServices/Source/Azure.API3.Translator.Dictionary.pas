...
type
  ...
  type
    TPartOfSpeachTag = (Unknown, Adjectives, Adverbs, Conjunctions, Determiners,
      ModalVerbs, Nouns, Prepositions, Pronouns, Verbs, Other);
    ...
    TTranslatedDictionaryResult = record
      ...
      Translations: TArray<TAzureTranslatorDictionaryTranslation> end;

      // TTranslatedDictionaryTexts = TArray<TTranslatedDictionaryText>;
    public
      constructor Create(AOwner: TComponent;
        ARegion: TAzureTranslationRegions = TAzureTranslationRegions.Global);
        reintroduce; overload;
      constructor Create(AOwner: TComponent; ARegionURLPrefix: string);
        reintroduce; overload;

      function Lookup(const AOriginalLanguage, AText, ATargetLanguage: string)
        : TTranslatedDictionaryResult;
    end;
...
function TAzureTranslatorDictionary.Lookup(const AOriginalLanguage, AText,
  ATargetLanguage: string): TTranslatedDictionaryResult;
begin
  if AText = '' then
    Exit();
  ...
  var
  Reader := RESTResponse.JSONReader;
  var
    CurrTranslation: Integer := -1;
  var
    CurrBackTranslation: Integer := -1;

  var
    InTranslationArray: Boolean := False;
  var
    InBackTranslationArray: Boolean := False;
    ...
    case Reader.TokenType of
      ...
      TJsonToken.StartArray:
        begin
          ...
          else if (SameText(CurrProperty, 'backTranslations')) then
          begin
      ...
      TJsonToken.EndArray:
        begin
          ...
          else if InTranslationArray then
      ...
      TJsonToken.StartObject:
        begin
          ...
          begin
            ...
            SetLength(Result.Translations[CurrTranslation].BackTranslations,
              CurrBackTranslation + 1);
          end
          else if InTranslationArray then
          begin
      ...
      TJsonToken.String:
        begin
          ...
          begin
            if (not InBackTranslationArray) then
            begin
              if SameText(CurrProperty, 'normalizedTarget') then
                Result.Translations[CurrTranslation].NormalizedTarget :=
                  Reader.Value.ToString
              else if SameText(CurrProperty, 'displayTarget') then
                Result.Translations[CurrTranslation].DisplayTarget :=
                  Reader.Value.ToString
              else if SameText(CurrProperty, 'prefixWord') then
                Result.Translations[CurrTranslation].PrefixWord :=
                  Reader.Value.ToString
              else if SameText(CurrProperty, 'posTag') then
              begin
                var
                CurrPosTag := Reader.Value.ToString;
{$REGION 'PosTag Validation'}
                if SameText(CurrPosTag, 'ADJ') then
                  Result.Translations[CurrTranslation].PartOfSpeach :=
                    TPartOfSpeachTag.Adjectives
                else if SameText(CurrPosTag, 'ADV') then
                  Result.Translations[CurrTranslation].PartOfSpeach :=
                    TPartOfSpeachTag.Adverbs
                else if SameText(CurrPosTag, 'CONJ') then
                  Result.Translations[CurrTranslation].PartOfSpeach :=
                    TPartOfSpeachTag.Conjunctions
                else if SameText(CurrPosTag, 'DET') then
                  Result.Translations[CurrTranslation].PartOfSpeach :=
                    TPartOfSpeachTag.Determiners
                else if SameText(CurrPosTag, 'MODAL') then
                  Result.Translations[CurrTranslation].PartOfSpeach :=
                    TPartOfSpeachTag.ModalVerbs
                else if SameText(CurrPosTag, 'NOUN') then
                  Result.Translations[CurrTranslation].PartOfSpeach :=
                    TPartOfSpeachTag.Nouns
                else if SameText(CurrPosTag, 'PREP') then
                  Result.Translations[CurrTranslation].PartOfSpeach :=
                    TPartOfSpeachTag.Prepositions
                else if SameText(CurrPosTag, 'PRON') then
                  Result.Translations[CurrTranslation].PartOfSpeach :=
                    TPartOfSpeachTag.Pronouns
                else if SameText(CurrPosTag, 'VERB') then
                  Result.Translations[CurrTranslation].PartOfSpeach :=
                    TPartOfSpeachTag.Verbs
                else if SameText(CurrPosTag, 'OTHER') then
                  Result.Translations[CurrTranslation].PartOfSpeach :=
                    TPartOfSpeachTag.Other
                else
                  Result.Translations[CurrTranslation].PartOfSpeach :=
                    TPartOfSpeachTag.Unknown;
{$ENDREGION}
              end;
            end
            else
            begin
              ...
              if SameText(CurrProperty, 'normalizedText') then
                Result.Translations[CurrTranslation].BackTranslations
                  [CurrBackTranslation].NormalizedText := Reader.Value.ToString
              else if SameText(CurrProperty, 'displayText') then
                Result.Translations[CurrTranslation].BackTranslations
                  [CurrBackTranslation].DisplayText := Reader.Value.ToString
              else

            end;
          end;
        end;
      TJsonToken.Float, TJsonToken.Integer:
        begin
          if InBackTranslationArray then
          begin
            if SameText(CurrProperty, 'numExamples') then
              Result.Translations[CurrTranslation].BackTranslations
                [CurrBackTranslation].NumExamples :=
                StrToIntDef(Reader.Value.ToString, -1)
            else if SameText(CurrProperty, 'frequencyCount') then
              Result.Translations[CurrTranslation].BackTranslations
                [CurrBackTranslation].FrequencyCount :=
                StrToIntDef(Reader.Value.ToString, -1);
          end
          else if InTranslationArray then
          begin
            if SameText(CurrProperty, 'confidence') then
              Result.Translations[CurrTranslation].Confidence :=
                StrToFloatDef(Reader.Value.ToString, -1);
          end
        end;
