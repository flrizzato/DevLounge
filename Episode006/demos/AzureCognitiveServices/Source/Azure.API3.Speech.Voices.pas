...
{$SCOPEDENUMS ON}
  ...
  public
    constructor Create(AOwner: TComponent; ARegion: TAzureSpeechRegion);
...
class function TAzureTextToSpeechVoices.VoiceGenderToString
  (aGender: TAzureVoiceGender): string;
begin
...
begin
  ...
  var
    InStyleList: Boolean := False;
  var
    InRolePlayList: Boolean := False;
  var
    InSecondaryLocaleList: Boolean := False;
  ...
  begin
    case Reader.TokenType of
      ...
      TJsonToken.StartArray:
        begin
          if SameText(CurrProperty, 'StyleList') then
          begin
            InStyleList := True;
          end
          else if SameText(CurrProperty, 'RolePlayList') then
          begin
            InRolePlayList := True;
          end
          else if SameText(CurrProperty, 'SecondaryLocaleList') then
          begin
      ...
      TJsonToken.EndArray:
        begin
          if InStyleList then
            InStyleList := False
          else if InRolePlayList then
            InRolePlayList := False
          else if InSecondaryLocaleList then
      ...
      TJsonToken.Integer, TJsonToken.Float, TJsonToken.String:
        begin
          if InStyleList then
          begin
            SetLength(FDataCache[CurrLanguage].StyleList,
              Length(FDataCache[CurrLanguage].StyleList) + 1);
            FDataCache[CurrLanguage].StyleList
              [Length(FDataCache[CurrLanguage].StyleList) - 1] :=
              Reader.Value.ToString;
          end
          else if InRolePlayList then
          begin
            SetLength(FDataCache[CurrLanguage].RolePlayList,
              Length(FDataCache[CurrLanguage].RolePlayList) + 1);
            FDataCache[CurrLanguage].RolePlayList
              [Length(FDataCache[CurrLanguage].RolePlayList) - 1] :=
              Reader.Value.ToString;
          end
          else if InSecondaryLocaleList then
          begin
            SetLength(FDataCache[CurrLanguage].SecondaryLocaleList,
              Length(FDataCache[CurrLanguage].SecondaryLocaleList) + 1);
            FDataCache[CurrLanguage].SecondaryLocaleList
              [Length(FDataCache[CurrLanguage].SecondaryLocaleList) - 1] :=
              Reader.Value.ToString;
          end
          else
          begin
            ...
            else if SameText(CurrProperty, 'DisplayName') then
              FDataCache[CurrLanguage].DisplayName := Reader.Value.ToString
            else if SameText(CurrProperty, 'LocalName') then
              FDataCache[CurrLanguage].LocalName := Reader.Value.ToString
            else if SameText(CurrProperty, 'ShortName') then
              FDataCache[CurrLanguage].ShortName := Reader.Value.ToString
            else if SameText(CurrProperty, 'SampleRateHertz') then
              FDataCache[CurrLanguage].SampleRateHertz :=
                StrToIntDef(Reader.Value.ToString, 0)
            else if SameText(CurrProperty, 'Locale') then
              FDataCache[CurrLanguage].Locale := Reader.Value.ToString
            else if SameText(CurrProperty, 'Gender') then
            begin
              var
              GenderStr := Reader.Value.ToString;
...
class function TAzureTextToSpeechVoices.VoiceStatusToString
  (aVoiceStatus: TAzureVoiceStaus): string;
begin
  Result := GetEnumName(TypeInfo(TAzureVoiceStaus), Integer(aVoiceStatus));
end;

class function TAzureTextToSpeechVoices.VoiceTypeToString
  (aVoiceType: TAzureVoiceType): string;
begin
  Result := GetEnumName(TypeInfo(TAzureVoiceType), Integer(aVoiceType));
end;

constructor TAzureTextToSpeechVoices.Create(AOwner: TComponent;
  ARegion: TAzureSpeechRegion);
begin
