...
{$SCOPEDENUMS ON}
  ...
  public
    constructor Create(AOwner: TComponent;
      const ARegion: TAzureSpeechRegion); overload;
    constructor Create(AOwner: TComponent;
      const ARegionURLPrefix: string); overload;
  end;
  ...
  public
    constructor Create(AOwner: TComponent; const ARegion: TAzureSpeechRegion);
      reintroduce; overload;
    constructor Create(AOwner: TComponent; const ARegionURLPrefix: string);
      reintroduce; overload;
    function SpeechToText(AInputStream: TStream;
      ...
      const aProfanity: TSpeechToTextProfanity = TSpeechToTextProfanity.masked)
      : TSpeechToTextResults; overload;
...
begin
  inherited Create(AOwner,
    TAzureURLBuilder.AZURE_COGNITIVE_SERVICES_SPEECH_TO_TEXT_BASE_URL,
    ARegionURLPrefix);
end;
...
begin
  ...
  RESTRequest.Resource :=
    'v1?language={language}&profanity={profanity}&format={format}';

  var
  Param := RESTRequest.Params.AddHeader('Content-Type', 'audio/wav');
...
begin
  ...
  RESTRequest.Params.ParameterByName('format').Value :=
    GetEnumName(TypeInfo(TSpeechToTextFormat), Integer(aFormat));
  RESTRequest.Params.ParameterByName('profanity').Value :=
    GetEnumName(TypeInfo(TSpeechToTextProfanity), Integer(aProfanity));

  Assert(AInputStream <> nil, 'Invalid Stream');

  if AInputStream is TStringStream then
  begin
    RESTRequest.Body.JSONWriter.WriteRaw(TStringStream(AInputStream)
      .DataString);
  end
  else
  begin
  ...
  if aFormat = TSpeechToTextFormat.simple then
  begin
    SetLength(Result, 1);
    ...
    begin
      case Reader.TokenType of
        ...
        TJsonToken.Integer, TJsonToken.String:
          begin
            ...
            else if SameText(CurrProperty, 'Offset') then
              Result[0].Offset := StrToIntDef(Reader.Value.ToString, 0)
            else if SameText(CurrProperty, 'Duration') then
              Result[0].Duration := StrToIntDef(Reader.Value.ToString, 0);
          end;
      end;
    end;
  end
  else if aFormat = TSpeechToTextFormat.detailed then
  begin
    SetLength(Result, 0);
    var
      CurrObjectID: Integer := -1;
    ...
    begin
      case Reader.TokenType of
        ...
        TJsonToken.StartObject:
          if not Reader.Path.Contains('.') then
          begin // Root object e.g. [0] or '' if not an response array.
            Inc(CurrObjectID);
        ...
        TJsonToken.Integer, TJsonToken.String:
          begin
            if SameText(CurrProperty, 'recognized') or
              SameText(CurrProperty, 'RecognitionStatus') then
              Result[CurrObjectID].RecognitionStatus := Reader.Value.ToString
            else if SameText(CurrProperty, 'text') or
              SameText(CurrProperty, 'DisplayText') or
              SameText(CurrProperty, 'Display') then
            begin
              if Result[CurrObjectID].DisplayText = '' then
                Result[CurrObjectID].DisplayText := Reader.Value.ToString;
            end
            else if SameText(CurrProperty, 'offset') then
              Result[CurrObjectID].Offset :=
                StrToIntDef(Reader.Value.ToString, 0)
            else if SameText(CurrProperty, 'duration') then
              Result[CurrObjectID].Duration :=
                StrToIntDef(Reader.Value.ToString, 0)
            else if SameText(CurrProperty, 'json') then
...
function TAzureSpeechToText.SpeechToText(aAudioFile: string;
  const aAudioLanguageCode: string = 'en-US';
  const aFormat: TSpeechToTextFormat = TSpeechToTextFormat.simple;
  const aProfanity: TSpeechToTextProfanity = TSpeechToTextProfanity.masked)
  : TSpeechToTextResults;
