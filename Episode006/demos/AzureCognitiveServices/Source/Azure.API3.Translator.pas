...
type
  ...
  public
    constructor Create(AOwner: TComponent;
      ARegion: TAzureTranslationRegions = TAzureTranslationRegions.
      Global); overload;
  ...
  public
    function Translate(AText: string; AOriginalLanguage: string;
...
begin
  ...
  for var I := 1 to Length(ATargetLangages) do
  begin
  ...
  for var I := 1 to Length(ATargetLangages) do
  begin
    RESTRequest.Params.ParameterByName('to' + I.ToString).Value :=
      ATargetLangages[Pred(I)];
  end;

  AccessToken.Token.AssignTokenToRequest(RESTRequest);
  Assert(RESTRequest.Params.IndexOf('Authorization') > -1,
    'Authorization Paramater missing');

  var
  Param := RESTRequest.Params.AddHeader('Content-Type',
  ...
  for var I := 0 to Length(ATargetLangages) - 1 do
  begin
    var
    CurrLang := ATargetLangages[I];
      ...
      TJsonToken.PropertyName:
        begin
          ...
          if (Reader.Value.ToString = 'language') and
            (Reader.Path.Contains('detectedLanguage')) then
          begin
            Reader.Read;
            Result.OriginalLanguage := Reader.Value.ToString;
          end
          else if (Reader.Value.ToString = 'score') and
            (Reader.Path.Contains('detectedLanguage')) then
          begin
            Reader.Read;
            Result.OriginalLanguageScore :=
              StrToFloatDef(Reader.Value.ToString, -1);
          end;

          if (Reader.Value.ToString = 'translations') then
          begin
        ...
        begin
          ...
          begin
            if SameText(CurrProperty, 'text') then
              Result.TranslatedTexts[CurrTranslation].Text :=
                Reader.Value.ToString
            else if SameText(CurrProperty, 'to') then
              Result.TranslatedTexts[CurrTranslation].Language :=
                Reader.Value.ToString;
          end;
        end;
    end;
...
begin
  Create(AOwner, TAzureURLBuilder.AZURE_TRANSLATOR_BASE_URL,
    TAzureURLBuilder.RegionToString(ARegion));
end;

constructor TAzureTranslatorBase.Create(AOwner: TComponent;
  ARegionURLPrefix: string);
begin
  inherited Create(AOwner, TAzureURLBuilder.AZURE_TRANSLATOR_BASE_URL,
    ARegionURLPrefix);
end;
