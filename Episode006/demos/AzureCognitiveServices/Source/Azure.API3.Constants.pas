...
{$SCOPEDENUMS ON}
  ...
  TAzureAuthRegion = (global, australiaeast, brazilsouth, canadacentral,
    centralindia, eastasia, eastus, japaneast, northeurope, southcentralus,
    southeastasia, uksouth, westcentralus, westeurope, westus, westus2);

  TAzureSpeechRegion = (australiaeast, brazilsouth, canadacentral, CentralUS,
    eastasia, eastus, EastUS2, FranceCentral, IndiaCentral, japaneast,
    KoreaCentral, NorthCentralUS, northeurope, SouthAfricaNorth, southcentralus,
    southeastasia, uksouth, westcentralus, westeurope, westus, westus2);

  // https://docs.microsoft.com/en-us/azure/cognitive-services/translator/reference/v3-0-reference
  TAzureTranslationRegions = (global,
    // api. Typically goes to the closet data center
    NorthAmerica, // api-nam
    Europe, // api-eur
    AsiaPacific); // api-apc

const
  TAzureTranslationRegionsStr: array [0 .. 3] of string = ('api', 'api-nam',
    'api-eur', 'api-apc');
...
type
  TAzureURLBuilder = class
    // Authorization
  const
    AZURE_COGNITIVE_SERVICES_AUTH_BASE_URL =
  ...
  const
    AZURE_COGNITIVE_SERVICES_TEXT_TO_SPEECH_BASE_URL =
      'https://{region}tts.speech.microsoft.com/cognitiveservices/';

  const
    AZURE_COGNITIVE_SERVICES_SPEECH_TO_TEXT_BASE_URL =
      'https://{region}stt.speech.microsoft.com/speech/recognition/conversation/cognitiveservices/';
    ...
    class function RegionToString(region: TAzureTranslationRegions)
      : string; overload;

    class function BuildServiceURL(const ServiceBaseURL: string;
      const region: TAzureAuthRegion): string; overload;
    class function BuildServiceURL(const ServiceBaseURL: string;
      const region: TAzureSpeechRegion): string; overload;
    class function BuildServiceURL(const ServiceBaseURL: string;
      const region: TAzureTranslationRegions): string; overload;
    class function BuildServiceURL(const ServiceBaseURL: string; region: string)
      : string; overload;
  end;
...
begin
  ...
  if Lowercase(region).StartsWith('api') then
  begin
    Result := StringReplace(ServiceBaseURL, '{region}', '',
      [rfReplaceAll, rfIgnoreCase]);
    Result := StringReplace(Result, 'api.', region,
      [rfReplaceAll, rfIgnoreCase]);
  end
  else
    Result := StringReplace(ServiceBaseURL, '{region}', region,
      [rfReplaceAll, rfIgnoreCase]);
end;
...
class function TAzureURLBuilder.RegionToString
  (region: TAzureAuthRegion): string;
begin
