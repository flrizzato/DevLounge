...
type
  ...
  type
    ...
    public
      ...
      procedure AssignTokenToRequest(aRequest: TRESTRequest);
  ...
  public
    ...
    /// </summary>
    constructor Create(AOwner: TComponent; const ARegion: TAzureAuthRegion);
      reintroduce; overload;
    /// <summary>
    /// Create an Access Token based on a specific Region - provide the string name of the region
    /// </summary>
    constructor Create(AOwner: TComponent; const ARegion: string);
      reintroduce; overload;
    ...
    /// </summary>
    property SubscriptionKey: string read GetSubscriptionKey
      write SetSubscriptionKey;
  ...
  public
    constructor Create(AOwner: TComponent;
      const AServiceBaseURL, ARegion: string); reintroduce;
    /// <summary>
...
constructor TAzureToken.Create(AOwner: TComponent;
  const ARegion: TAzureAuthRegion);
begin
...
begin
  ...
  FRegion := Value;
  RESTClient.BaseURL := TAzureURLBuilder.BuildServiceURL
    (TAzureURLBuilder.AZURE_COGNITIVE_SERVICES_AUTH_BASE_URL, Region);
end;
...
begin
  ...
  begin
    FValue := Value;
    FExpiry := now + EncodeTime(0, 8, 0, 0);
    // Token valid for 10, but set to 8 (MS recommended)
  end;
end;

{ TAzureTransport }

constructor TAzureTransport.Create(AOwner: TComponent;
  const AServiceBaseURL, ARegion: string);
var
  Param: TRESTRequestParameter;
begin
  inherited Create(AOwner);
...
begin
  ...
  FRESTRequest.AssignedValues :=
    [TCustomRESTRequest.TAssignedValue.rvConnectTimeout,
    TCustomRESTRequest.TAssignedValue.rvHandleRedirects];
end;
