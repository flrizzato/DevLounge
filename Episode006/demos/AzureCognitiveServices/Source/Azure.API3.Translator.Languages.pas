...
type
  ...
  public
    ...
    procedure LoadToString(aStrings: TStrings;
      aFormat: TAzureLanguageFormat = TAzureLanguageFormat.name);
    function LanguageStringToID(aLanguageString: string;
      aInputFormat: TAzureLanguageFormat = TAzureLanguageFormat.name): string;
    function LanguageIDToString(aLanguageID: string;
      aOutFormat: TAzureLanguageFormat = TAzureLanguageFormat.name): string;
  end;
  ...
  public
    ...
    constructor Create(AOwner: TComponent; ARegionURLPrefix: string);
      reintroduce; overload;
    property Dictionaries: TAzureDictionaryLanguagesMap
      read GetAzureDictionaries;
...
begin
  ...
  begin
    ...
    var
      InTranslationArrayDetail: Boolean := False;
      ...
      case Reader.TokenType of
        TJsonToken.PropertyName:
          begin
            ...
            if (Reader.Value.ToString = 'translation') then
            begin
              InTranslationArray := True;
            end
            else if InTranslationArray then
            begin
              if InTranslationArrayDetail = False then
              begin
        ...
        TJsonToken.String:
          begin
            ...
            begin
              ...
              else if SameText(CurrProperty, 'nativeName') then
                FDataCache[CurrTranslation].nativeName := Reader.Value.ToString
              else if SameText(CurrProperty, 'dir') then
...
begin
  for var I := 0 to Pred(Length(Languages)) do
  begin
    if FDataCache[I].id = aLanguageID then
      case aOutFormat of
        TAzureLanguageFormat.id:
          Exit(FDataCache[I].id);
        TAzureLanguageFormat.nativeName:
          Exit(FDataCache[I].nativeName);
      else { name }
        Exit(FDataCache[I].name);
      end;
  end;
  Result := '<unknown>';
end;
...
begin
  ...
  begin
    case aInputFormat of
      TAzureLanguageFormat.id:
        if FDataCache[I].id = aLanguageString then
          Exit(FDataCache[I].id);

      TAzureLanguageFormat.nativeName:
        if FDataCache[I].nativeName = aLanguageString then
          Exit(FDataCache[I].id);

    else { name }
      if FDataCache[I].name = aLanguageString then
        Exit(FDataCache[I].id);
    end;
  end;
...
procedure TAzureTranslatorLanguages.LoadToString(aStrings: TStrings;
  aFormat: TAzureLanguageFormat = TAzureLanguageFormat.name);
begin
  aStrings.Clear;
  for var I := 0 to Pred(Length(Languages)) do
  begin
    case aFormat of
      TAzureLanguageFormat.id:
        aStrings.Add(FDataCache[I].id);
      TAzureLanguageFormat.nativeName:
        aStrings.Add(FDataCache[I].nativeName);
    else { name }
      aStrings.Add(FDataCache[I].name);
    end;
  end;
end;
...
function TAzureTranslatorDictionaryScope.DictionaryByID(aLanguageID: string)
  : TAzureDictionaryLanguageMap;
begin
  for var Dict in Dictionaries do
  begin
...
function TAzureTranslatorDictionaryScope.GetAzureDictionaries
  : TAzureDictionaryLanguagesMap;
begin
  if Length(FDataCache) = 0 then
  begin
    RESTRequest.Execute;
    ...
    var
      CurrLanguage: Integer := -1;
    var
      InDictionaryItems: Boolean := False;
    var
      InDictionaryArrayDetail: Boolean := False;

    var
      CurrTranslation: Integer := -1;
    var
      InTranslationItems: Boolean := False;
    ...
    begin
      case Reader.TokenType of
        TJsonToken.PropertyName:
          begin
            CurrProperty := Reader.Value.ToString;
            if (Reader.Value.ToString = 'dictionary') then
            begin
              InDictionaryItems := True;
            end
            else if InDictionaryItems then
            begin
              if InDictionaryArrayDetail = False then
              begin
        ...
        TJsonToken.StartObject:
          begin
            ...
            begin
              Inc(CurrTranslation);
              SetLength(FDataCache[CurrLanguage].translations,
                CurrTranslation + 1);
            end;
          end;
        TJsonToken.EndObject:
          begin
            if InDictionaryItems and (not InTranslationItems) then
            begin
            ...
            begin
              ...
              begin
                ...
                if SameText(CurrProperty, 'code') then
                  FDataCache[CurrLanguage].translations[CurrTranslation] :=
