...
{$SCOPEDENUMS ON}
  ...
  public
    ...
    constructor Create(AOwner: TComponent; const ARegionURLPrefix: string);
      reintroduce; overload;
    function TextToSpeech(const AText: string; AOutputStream: TMemoryStream;
      aDocumentLanguageCode: string = 'en-US';
      aAzureVoiceName: string = 'en-US-AriaNeural'): Boolean;
    function RawTextToSpeech(const AText: string;
  ...
  public
    ...
    property DocumentLanguage: string read FDocumentLanguage
      write SetDocumentLanguage;
  end;
...
begin
  ...
  var
  Param := RESTRequest.Params.AddHeader('Content-Type', 'application/ssml+xml');
  Param.Options := [TRESTRequestParameterOption.poDoNotEncode];

  Param := RESTRequest.Params.AddHeader('X-Microsoft-OutputFormat',
...
function TAzureTextToSpeech.TextToSpeech(const AText: string;
  AOutputStream: TMemoryStream; aDocumentLanguageCode: string = 'en-US';
  aAzureVoiceName: string = 'en-US-AriaNeural'): Boolean;
begin
  var
  aBody := '<speak version=''1.0'' xml:lang=''%s''>' + '<voice name=''%s''>' +
    '%s' + // Text
    '</voice>' + '</speak>';
...
begin
  ...
  if Result then
    AOutputStream.WriteData(RESTResponse.RawBytes,
      Length(RESTResponse.RawBytes));
end;
...
begin // xmlns="http://www.w3.org/2001/10/synthesis"
  Result := Format('<speak version=''1.0''  xml:lang=''%s''>',
    [DocumentLanguage]) + FInternalText.Text + '</speak>';
end;
