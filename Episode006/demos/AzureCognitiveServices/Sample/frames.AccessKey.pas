...
begin
  ...
  begin
    if cbRegion.Selected = nil then
      FAzureToken.Region := TAzureURLBuilder.RegionToString
...
begin
  ...
  try
    ...
    for var Region := Low(TAzureAuthRegion) to High(TAzureAuthRegion) do
    begin
      var
      RegionName := TAzureURLBuilder.RegionToString(Region);
      if RegionName > '' then
...
begin
  if Value = '' then
    cbRegion.ItemIndex := cbRegion.Items.IndexOf
      (TAzureURLBuilder.RegionToString(TAzureAuthRegion.global))
  else
