...
begin
	...
	try
		...
		frameAccessKeyTranslator.Region := ini.ReadString('Translation',
			'Region', '');
...
begin
	if tcMain.ActiveTab = tabTranslator then
		FrameTranslateText1.Initialize(
			function(): TAzureToken
			begin
				Result := frameAccessKeyTranslator.AzureToken;
			end)
	else if tcMain.ActiveTab = tabDictionary then
		FrameDictionary1.Initialize(
			function(): TAzureToken
			begin
				Result := frameAccessKeyTranslator.AzureToken;
			end)
	else if tcMain.ActiveTab = tabTextToSpeech then
	begin
		FrameTextToSpeech1.Initialize(
			function(): TAzureToken
			begin
				Result := frameAccessKeySpeech.AzureToken;
			end);
	end
	else if tcMain.ActiveTab = tabSpeechToText then
		FrameSpeechToText1.Initialize(
			function(): TAzureToken
			begin
				Result := frameAccessKeySpeech.AzureToken;
			end);
