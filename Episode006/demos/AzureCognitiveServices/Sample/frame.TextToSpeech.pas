...
begin
  ...
  try
    var
    aAudio := TAzureTextToSpeech.Create(Self, FTokenProc.Region);
    ...
    var
    JokeText := TAzureTextBuilder.Create();
    try
      ...
      if aAudio.RawTextToSpeech(JokeText.RawText, MS) then
      begin
        var
        TempFile := TPath.Combine(TPath.GetLibraryPath, 'TestAudio.mp3');
...
begin
  var
  FVoices := TAzureTextToSpeechVoices.Create(Self,
    TAzureSpeechRegion(cbVoicesRegions.ItemIndex));
  try
    ...
    try
      ...
      MemoLog.Lines.Clear;
      var
      Voices := FVoices.Voices;
      for var Voice in Voices do
      begin
        ...
        if Length(Voice.SecondaryLocaleList) > 0 then
          MemoLog.Lines.Add(Format(' - %s has %d secondary locals',
            [Voice.LocalName, Length(Voice.SecondaryLocaleList)]));
        MemoLog.Lines.Add('Short Name - ' + Voice.ShortName);
        MemoLog.Lines.Add('Gender - ' +
          TAzureTextToSpeechVoices.VoiceGenderToString(Voice.Gender));
        MemoLog.Lines.Add('Status - ' +
          TAzureTextToSpeechVoices.VoiceStatusToString(Voice.Status));
        MemoLog.Lines.Add('Type - ' + TAzureTextToSpeechVoices.VoiceTypeToString
          (Voice.VoiceType));
...
begin
  ...
  if cbVoicesRegions.Items.Count = 0 then
  begin
  ...
  var
    Task: ITask := TTask.Create(procedure()begin Sleep(100);
      TThread.Synchronize(TThread.Current,
      procedure begin cbVoicesRegions.ItemIndex := cbVoicesRegions.Items.IndexOf
      (FTokenProc.Region); end); end);
  Task.Start;
end;
