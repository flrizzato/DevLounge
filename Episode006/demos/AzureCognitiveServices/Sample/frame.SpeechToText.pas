...
begin
  var
  STT := TAzureSpeechToText.Create(Self, FTokenProc.Region);
  try
    STT.AccessToken := FTokenProc;

    var
      aFormat: TAzureSpeechToText.TSpeechToTextFormat;
    ...
    var
    R := STT.SpeechToText(edtFilePath.Text, edtCountryCode.Text, aFormat);

    for var I := 0 to Length(R) - 1 do
    begin
...
begin
  ...
  var
  FileSetup := SetupAudioFile(TPath.Combine(TPath.GetLibraryPath, 'audio.wav'))
    or SetupAudioFile(TPath.Combine(TPath.GetLibraryPath, 'audio.ogg')) or
    SetupAudioFile(TPath.Combine(TPath.GetDocumentsPath, 'audio.wav')) or
    SetupAudioFile(TPath.Combine(TPath.GetDocumentsPath, 'audio.ogg'));
